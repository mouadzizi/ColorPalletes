...
import { Text, TextInput, StyleSheet, SafeAreaView, TouchableOpacity, FlatList, Switch, View, Alert} from 'react-native'
...
export default function moodal({navigation}) {
    ...
    const HandleNavigation = useCallback( () => {
        if(!title)
        {
            Alert.alert('please enter a pallete Color name');
        } else {
            const newCollorPallete = {
                paletteName : title,
                colors: selectedColors,
              };
            navigation.navigate('Home', {newCollorPallete});
        }
      }, [title, selectedColors]);
    
      const HandleValueChangeSwitch = useCallback((value, color)=> {
        if (value === true){
    ...
    return (
        ...
        style={styles.container}>
            <Text style={styles.title}>Add new Color panelle</Text>

            <TextInput 
            style={styles.input}
            placeholder='Enter a pallete name'
            textAlign='center'
            ...
            <FlatList
                ...
                <View style={styles.container2}>
                    ...
                    onValueChange={selected => {HandleValueChangeSwitch(selected, item)}}/>
                </View>
                }
            />
